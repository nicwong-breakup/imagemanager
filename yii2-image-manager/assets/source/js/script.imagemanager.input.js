var imageManagerInput = {
				...
				sModalHtml += '<div class="modal-dialog modal-lg">';
					sModalHtml += '<div class="modal-content">';
						sModalHtml += '<div class="modal-header">';
							sModalHtml += '<h4>Media</h4>';
							sModalHtml += '<button aria-hidden="true" data-dismiss="modal" class="close close-modal-imagemanager" type="button">&times;</button>';
	...
	openModal: function(inputId, multi){
		//get selected item
		...
		var imageManagerUrl = imageManagerInput.baseUrl+queryStringStartCharacter+"view-mode=iframe&input-id="+inputId+srcImageIdQueryString+"&multi="+multi;
		//set iframe path
...
$(document).ready(function () {
	...
	$(document).on("click", ".open-modal-imagemanager", function () {
		...
		//open selector id
		imageManagerInput.openModal(inputId, multi);
	});	
