		...
		switch(imageManagerModule.selectType){
			...
			case "input":
				...
				for($i = 0; $i < imageManagerModule.selectedImage.length; $i++){
					...
					{
						child.find('img').addClass('hide');
						child.find('video').attr('src',currentImage.image).removeClass('hide');
					}
					if(currentImage.type == 'image')
					{
						child.find('video').addClass('hide');
						child.find('img').attr('src',currentImage.image).removeClass('hide');
					}
		...
		$.ajax({
			...
			success: function (responseData, textStatus, jqXHR) {
				...
				//else set data
					...
					{
						var videoHtml = "";
						videoHtml += `<video id="video-thumbnail">
							<source type="video/mp4" src="${responseData.image}" alt="${responseData.fileName}">
						</video>`;
						$("#module-imagemanager .image-info .preview-video").removeClass("hide");
						...
						// $("#module-imagemanager .image-info .thumbnail").html(videoHtml);

						// const _VIDEO = document.querySelector("#video-thumbnail"),
						// _CANVAS = document.querySelector("#canvas-thumbnail"),
						// _CANVAS_CTX = _CANVAS.getContext("2d");
						// _CANVAS_CTX.drawImage(_VIDEO, 0, 0,_VIDEO.videoWidth, _VIDEO.videoHeight);						
						$("#module-imagemanager .image-info .thumbnail").html(videoHtml).attr('data-video', responseData.image);

						
					}
					else{
						$("#module-imagemanager .image-info .preview-video").addClass('hide');
						$("#module-imagemanager .image-info .dimensions .dimension-width").text(responseData.dimensionWidth);
						$("#module-imagemanager .image-info .dimensions .dimension-height").text(responseData.dimensionHeight);
...
$(document).ready(function () {
	...
	$(document).on('click', "#module-imagemanager .image-info .preview-video", function(e){
		...
		const fullFile = $('#module-imagemanager .image-info .thumbnail').attr('data-video');
		$('#my-video').find('video').find('source').attr('src',fullFile);
		player = videojs('my-video');
		player.src({ src: fullFile});
		player.load();
