...
class ManagerController extends Controller {
	...
	public function actionIndex() {
		...
		$searchModel = new ImageManagerSearch($defaultImageId);
	...
	public function actionUpload() {
		...
		$env = require Yii::getAlias('@config')."/env.php";
		$s3 = Yii::$app->get('s3');
		...
		if (isset($_FILES['imagemanagerFiles']['tmp_name'])) {
			...
			foreach ($_FILES['imagemanagerFiles']['tmp_name'] AS $key => $sTempFile) {
				...
				$actual_name = pathinfo($sFileName,PATHINFO_FILENAME);
				...
				$mediaModel = ImageManager::find()->where(['fileName' => $sFileName])->all();

				$i = 1;
				if($s3->exist($sMediaPath.'/'.$sFileName)) {
					$sFileName = $original_name.'_1.'.$sFileExtension;
				}
				if($sFileName)
				$result = $s3->upload($sMediaPath.'/'.$sFileName, $sTempFile);

				$url = isset($result['ObjectURL']) ? $result['ObjectURL'] : '';
				foreach ($result as $r) {
					if (is_array($r)) {
					  if (array_key_exists('statusCode',$r)) {
						...
						{
							$type = "";
							$realUrl = str_replace($env['s3Domain'].$sMediaPath.'/', '', $url);
							$imageApiUrl = $env['apiDomain'].'image/'.$realUrl;

							if(strpos($sFileType, 'image') !== false) {
								$type = "image";
							}
							...
							if($url) 
							{
								...
								$model->url = $imageApiUrl;
								$model->type = $type;
								$model->fileName = $realUrl;
								$model->save();
								$bSuccess = true;
							}
						}
...
//                       
	...
	public function actionDelete() {
		...
		//delete record
		if ($model->delete()) {
			$result = $s3->delete($sMediaPath.'/'.$model->fileName);
			$return['delete'] = true;
		}
		return $return;
	}
